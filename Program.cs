...
void GenerateFiles(string folderPath, int numberOfFiles)
{
    for (var fileCount = 0; fileCount < numberOfFiles; fileCount++)
    {
        ...
        Console.WriteLine($"Create file {newFileName} in directory {folderPath} with a date of {creationTime}");
...
string CreateDirectory(string name) => Directory.CreateDirectory(Path.Combine(name, Guid.NewGuid().ToString()).ToString()).ToString();